...
### Validation Results
A sequence of machine learning models is developed and tuned to optimize Median Absolute Percentage Error (MdAPE), with Mean Absolute Percentage Error (MAPE), R<sup>2</sup>, and other metrics reported for additional reference. The table below summarizes the top-performing model from each category based on validation MdAPE.
...
The two gradient boosting models achieve sub-8% MdAPE on the validation set and are further evaluated for predictive accuracy and stability.
