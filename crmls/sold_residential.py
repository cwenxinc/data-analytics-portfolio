...
sold_residential.shape # 412808 records retained
...
null_table.loc[['OriginalListPrice', 'ListPrice', 'ClosePrice',              # 18% (738 records) missing original list price, 2 records missing close price
                'DaysOnMarket', 
                'LivingArea', 'BedroomsTotal', 'BathroomsTotalInteger',      # 6% (238 records) missing living area
                'CloseDate', 'PurchaseContractDate', 'ListingContractDate']] # 5% (195 records) missing purchase date, 1 record missing listing date
...
sold_valid.shape # 411564 records retained
...
sold_core_not_missing.shape # 410621 records retained
...
sold_clean.shape[1] # 17 fields removed
...
# remove outliers
sold_filtered = sold_clean[~sold_clean['outlier_flag']]
sold_filtered.shape # 288053 records retained
sold_filtered.shape[0] / sold_clean.shape[0] # 70.1% retained from clean data
sold_filtered.shape[0] / sold_full.shape[0]  # 69.8% retained from raw data
sold_filtered[metrics].describe(percentiles=[0.1, 0.25, 0.5, 0.75, 0.9])

sold_filtered = sold_filtered.drop(columns='outlier_flag')
sold_filtered.to_csv("clean-data/sold_residential_metrics_outliers_removed.csv", index=False)