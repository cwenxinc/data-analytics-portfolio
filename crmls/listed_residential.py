...
listed_residential.shape # 568933 records retained
...
mortgage.columns = ['date', 'rate_30yr_fixed'] # most recent rate dates to 2026-05-14
...
null_table.loc[['OriginalListPrice', 'ListPrice',                       # 14% (788 records) missing original list price
                'DaysOnMarket', 
                'LivingArea', 'BedroomsTotal', 'BathroomsTotalInteger', # 11% (615 records) missing living area
                'ListingContractDate']]                                 
...
listed_valid.shape # 538016 records retained

# (ii) remove records missing core values
listed_core_not_missing = listed_valid.dropna(subset=['OriginalListPrice', 'LivingArea'])
listed_core_not_missing.shape                           # 536861 records retained
listed_core_not_missing.shape[0] / listed_full.shape[0] # 94% of raw records retained
...
listed_clean.shape[1] # 62 fields retained

listed_clean.shape # (536861, 62)
...
# remove outliers
listed_filtered = listed_clean[~listed_clean['outlier_flag']]
listed_filtered.shape # 417060 records retained
listed_filtered.shape[0] / listed_clean.shape[0] # 77.7% retained from clean data
listed_filtered.shape[0] / listed_full.shape[0]  # 73.3% retained from raw data
listed_filtered[metrics].describe(percentiles=[0.1, 0.25, 0.5, 0.75, 0.9])

listed_filtered = listed_filtered.drop(columns='outlier_flag')
listed_filtered.to_csv("clean-data/listed_residential_metrics_outliers_removed.csv", index=False)